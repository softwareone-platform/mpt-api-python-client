...
mis-decodes, though a standalone `%` is tolerated. `RQLQuery` percent-encodes every value
and every filter field-path segment it renders:
...
- The guarantee covers the filter expression only. `order_by()` and `select()` field names
  are interpolated into the query string directly and are **not** encoded, so a reserved
  character in one of those names would not survive. They are developer-supplied field
  paths rather than data, so in practice they are plain identifiers.
