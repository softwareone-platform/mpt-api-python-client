...
# accepts, exactly as the paged reader's json.loads does, so acceptance cannot depend on
# where a chunk boundary lands. Anything else cannot become a value however much body
# follows, so it is a malformed envelope rather than a token still arriving.
VALUE_STARTS = '"{[-0123456789tfnIN'
...
# Tokens a failed decode may be an unfinished prefix of: the JSON literals and the
# decoder's non-standard constants. A lone minus sign — which only a number's digits or
# the rest of -Infinity can follow — is an unfinished prefix of the latter.
INCOMPLETE_VALUE_PREFIXES = ("null", "true", "false", "NaN", "Infinity", "-Infinity")
...
class JSONEnvelopeParser:
    ...
    def __init__(self, data_field: str = MPT_DATA_FIELD) -> None:
        ...
        self._position = 0
        self._member_key = ""
        self._consumed_members: set[str] = set()
        self._deferred_chunks: list[str] = []
    ...
    def feed(self, chunk: str) -> Iterator[StreamEvent]:
        ...
        """
        if self._decode_error is not None and not chunk.strip(JSON_WHITESPACE):
            # Whitespace cannot complete a token the buffer ran out inside — an open
            # string absorbs it as content, nothing else changes — so hold the chunk
            # instead of re-scanning the whole buffered prefix per keep-alive.
            self._deferred_chunks.append(chunk)
            return
        if self._deferred_chunks:
            chunk = "".join([*self._deferred_chunks, chunk])
    ...
    def _read_member_key(self) -> None:
        ...
        self._track_member(member_key)
        self._member_key = member_key
        self._state = _State.MEMBER_COLON

    def _track_member(self, member_key: str) -> None:
        """Reject a repeated occurrence of a member the parser acts on.

        For `json.loads` — and so for the paged reader — the last occurrence of a
        repeated member wins, but a streaming reader cannot take back the records or
        the total an earlier occurrence already emitted. A repeated record array or
        ``$meta`` is therefore rejected as malformed; other members are decoded and
        discarded, so their repeats stay as harmless as they are to `json.loads`.

        Args:
            member_key: Envelope member name just read.

        Raises:
            JSONDecodeError: If the record array member or ``$meta`` repeats.
        """
        if member_key not in {self._data_field, MPT_META_FIELD}:
            return
        if member_key in self._consumed_members:
            raise self._malformed_envelope(f"Duplicate envelope member '{member_key}'")
        self._consumed_members.add(member_key)
    ...
    def _decode(self) -> Any:
        ...
        if self._number_may_continue(decoded, end):
            # The decode succeeded, so any failure stored by an earlier attempt is
            # superseded — left in place, close() would re-raise the stale error.
    ...
    def _number_may_continue(self, decoded: Any, end: int) -> bool:
        ...
        self-terminating, so a prefix of one cannot decode at all — including a boolean,
        an ``int`` subclass to ``isinstance`` but decoded from a literal nothing extends.
        Waiting is justified only while the leftover characters can still become part of
        the number, so a dead continuation like ``12.}`` fails fast instead.

        Args:
            decoded: Value the decoder returned.
        ...
        if end >= len(self._buffer):
            return True
        return NUMBER_CONTINUATION.match(self._buffer, end) is not None
