...
@pytest.fixture
def created_licensee(mpt_client, licensee, licensee_account_id, account_icon):
    new_licensee_request_data = licensee(
        name="E2E Created licensee",
        account_id=licensee_account_id,
    )

    new_licensee = mpt_client.accounts.licensees.create(new_licensee_request_data, logo=account_icon)
...
def test_update_licensee(mpt_client, licensee, account_icon, created_licensee):
    updated_licensee_data = licensee(name="E2E Updated Licensee")
...
def test_update_licensee_not_found(mpt_client, licensee, account_icon, invalid_licensee_id):
    updated_licensee_data = licensee(name="Nonexistent Licensee")

    with pytest.raises(MPTAPIError, match=r"404 Not Found"):
        mpt_client.accounts.licensees.update(invalid_licensee_id, updated_licensee_data, logo=account_icon)
