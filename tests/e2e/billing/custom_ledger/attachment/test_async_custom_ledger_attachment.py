...
async def created_custom_ledger_attachment(
    async_mpt_ops, custom_ledger_attachment_factory, billing_custom_ledger_id, pdf_fd
):
    new_custom_ledger_attachment_request_data = custom_ledger_attachment_factory(
        name="E2E Created Custom Ledger Attachment",
    )
    custom_ledger_attachments = async_mpt_ops.billing.custom_ledgers.attachments(
        billing_custom_ledger_id
    )
...
@pytest.fixture
def custom_ledger_attachments(async_mpt_ops, billing_custom_ledger_id):
    return async_mpt_ops.billing.custom_ledgers.attachments(billing_custom_ledger_id)
...
async def test_filter_custom_ledger_attachments(custom_ledger_attachments, custom_ledger_attachment_id):
    select_fields = ["-price"]
