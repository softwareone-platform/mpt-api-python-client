...
@pytest.fixture
def created_invoice_attachment(
    mpt_ops,
    invoice_attachments,
...
@pytest.fixture
def invoice_attachments(mpt_ops, billing_invoice_id):
    return mpt_ops.billing.invoices.attachments(billing_invoice_id)


def test_get_invoice_attachment_by_id(invoice_attachments, invoice_attachment_id):
    result = invoice_attachments.get(invoice_attachment_id)

    assert result is not None


def test_get_invoice_attachment_by_id_not_found(
    invoice_attachments,
    invalid_invoice_attachment_id,
):
    with pytest.raises(MPTAPIError, match=r"404 Not Found"):
        invoice_attachments.get(invalid_invoice_attachment_id)


def test_list_invoice_attachments(invoice_attachments):
    limit = 10

    result = invoice_attachments.fetch_page(limit=limit)

    assert len(result) > 0


def test_filter_invoice_attachments(invoice_attachments, invoice_attachment_id):
...
def test_update_billing_invoice_attachment(
    invoice_attachments,
    ...
    result = invoice_attachments.update(
...
def test_delete_billing_invoice_attachment(
    invoice_attachments,
    created_invoice_attachment,
):
    result = created_invoice_attachment

    invoice_attachments.delete(result.id)


def test_download_billing_invoice_attachment(
    invoice_attachments,
    invoice_attachment_id,
):
    result = invoice_attachments.download(invoice_attachment_id)
