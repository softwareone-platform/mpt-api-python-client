...
@pytest.fixture
async def created_ledger_attachment(async_mpt_ops, ledger_attachment_factory, billing_ledger_id, pdf_fd):
    new_ledger_attachment_request_data = ledger_attachment_factory(
        name="E2E Created Ledger Attachment",
    )
    ledger_attachments = async_mpt_ops.billing.ledgers.attachments(billing_ledger_id)
    created_ledger = await ledger_attachments.create(new_ledger_attachment_request_data, file=pdf_fd)

    yield created_ledger

    try:
        ledger_attachments.delete(created_ledger.id)
    except MPTAPIError as error:
        print("TEARDOWN - Unable to delete ledger attachment: %s", error.title)  # noqa: WPS421


@pytest.fixture
def ledger_attachments(async_mpt_ops, billing_ledger_id):
    return async_mpt_ops.billing.ledgers.attachments(billing_ledger_id)
...
async def test_get_ledger_attachment_by_id_not_found(ledger_attachments, invalid_ledger_attachment_id):
...
async def test_filter_ledger_attachments(ledger_attachments, ledger_attachment_id):
    ...
    filtered_attachments = (
        ...
        .filter(RQLQuery(name="E2E Seeded Billing Ledger Attachment"))
...
async def test_delete_billing_ledger_attachment(ledger_attachments, created_ledger_attachment):
    result = await ledger_attachments.delete(created_ledger_attachment.id)
