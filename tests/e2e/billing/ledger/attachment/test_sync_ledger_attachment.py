...
@pytest.fixture
def created_ledger_attachment(mpt_ops, ledger_attachment_factory, billing_ledger_id, pdf_fd):
    new_ledger_attachment_request_data = ledger_attachment_factory(
        name="E2E Created Ledger Attachment",
    )
    ledger_attachments = mpt_ops.billing.ledgers.attachments(billing_ledger_id)

    created_ledger = ledger_attachments.create(new_ledger_attachment_request_data, file=pdf_fd)
...
@pytest.fixture
def ledger_attachments(mpt_ops, billing_ledger_id):
    return mpt_ops.billing.ledgers.attachments(billing_ledger_id)
...
def test_delete_billing_ledger_attachment(ledger_attachments, created_ledger_attachment):
    result = ledger_attachments.delete(created_ledger_attachment.id)
