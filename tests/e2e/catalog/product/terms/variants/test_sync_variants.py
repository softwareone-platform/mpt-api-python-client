...
def test_create_variant(created_variant, variant_data):
    assert created_variant.name == variant_data["name"]
...
def test_update_variant(vendor_variant_service, created_variant):
    ...
    variant = vendor_variant_service.update(created_variant.id, update_data)
    assert variant.name == "e2e - delete me (updated)"


def test_get_variant(vendor_variant_service, variant_id):
    variant = vendor_variant_service.get(variant_id)
...
def test_iterate_variants(vendor_variant_service, created_variant):
    variants = list(vendor_variant_service.iterate())
