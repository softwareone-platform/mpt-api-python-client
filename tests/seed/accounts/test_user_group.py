...
def user_groups_service():
    return AsyncMock(spec=AsyncUserGroupsService)


async def test_get_user_group(
    context: Context, operations_client, user_group, user_groups_service
) -> None:
...
async def test_get_user_group_without_id(context: Context) -> None:
    result = await get_user_group(context=context)
    assert result is None


async def test_init_user_group(
    context: Context, operations_client, user_groups_service, user_group
) -> None:
    user_groups_service.create.return_value = user_group
    operations_client.accounts.user_groups = user_groups_service

    result = await init_user_group(context, mpt_operations=operations_client)

    assert result == user_group
    user_groups_service.create.assert_called_once()


def test_build_user_group_data(context: Context) -> None:
...
async def test_init_user_group_create_new(
    context: Context, operations_client, user_groups_service, user_group
) -> None:
    user_groups_service.create.return_value = user_group
    operations_client.accounts.user_groups = user_groups_service
    with patch("seed.accounts.user_group.get_user_group", return_value=None):
        result = await init_user_group(context, mpt_operations=operations_client)
...
async def test_seed_user_group() -> None:
    with (
        patch("seed.accounts.user_group.init_user_group", new_callable=AsyncMock) as mock_init_user_group,
    ):
