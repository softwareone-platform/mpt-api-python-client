...
async def test_create_product(  # noqa: WPS211
    mocker, context: Context, vendor_client, product
):
...
async def test_create_product_item(mocker, vendor_client, context_with_product):
    context = context_with_product
    context["catalog.unit.id"] = "unit-1"
    context["catalog.product.item_group.id"] = "ig-1"

    create_mock = mocker.AsyncMock(return_value={"id": "item-1"})
    vendor_client.catalog.items.create = create_mock
    result = await create_product_item(context, vendor_client)
