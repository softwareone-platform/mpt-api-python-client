...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param('{"elapsed": 1e+', "3", id="signed exponent - split after the sign"),
    ],
)
...
def test_keepalive_inside_a_string_stays_content(parse_envelope):
    # A whitespace-only chunk arriving while a token is in flight is held rather than
    # re-scanned; when it lands inside an open string it is still string content.
    chunks = ['{"data": [{"id": "ID', " ", '-1", "name": "Order 1"}]}']

    result = parse_envelope(chunks)

    assert result == [StreamedRecord({"id": "ID -1", "name": "Order 1"})]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param('{"data": [], "x": 1, "x": 2}', id="repeated unwatched member"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # The second chunk decodes -12 and supersedes the failure the lone minus stored,
        # so the cut mid-number reports the envelope, not a stale 'Expecting value'.
        pytest.param(['{"n": -', "12"], UNTERMINATED_MATCH, id="number split then cut"),
    ],
)
...
@pytest.mark.parametrize(
    "body",
    [
        pytest.param('{"x": 1e}, "data": []}', id="dead exponent"),
...
def test_feed_rejects_a_dead_number_continuation(body):
    # A continuation character the number grammar cannot extend through must not keep
    # the value waiting for more body: the decoded prefix ends the token, so the tail
    # is rejected where a separator was due.
    parser = JSONEnvelopeParser()

    with pytest.raises(json.JSONDecodeError, match="Expected ',' or '}'"):
        list(parser.feed(body))
...
@pytest.mark.parametrize(
    "body",
    [
        pytest.param('{"data": [], "data": []}', id="repeated record array"),
        pytest.param('{"$meta": {}, "data": [], "$meta": {}}', id="repeated meta"),
    ],
)
def test_feed_rejects_a_repeated_special_member(body):
    # The paged reader keeps the last occurrence of a repeated member, but a streaming
    # reader cannot take back records or a total already emitted, so a repeat of a
    # member the parser acts on is malformed rather than silently double-processed.
    parser = JSONEnvelopeParser()

    with pytest.raises(json.JSONDecodeError, match="Duplicate envelope member"):
        list(parser.feed(body))
...
# raw_decode accepts the non-standard Infinity/-Infinity/NaN exactly as the paged
# reader's json.loads does, so acceptance must not depend on where a chunk boundary
# lands inside the constant.
@pytest.mark.parametrize(
    "chunks",
    [
        pytest.param([f'{{"x": Infinity, "data": [{FIRST_RECORD_JSON}]}}'], id="one chunk"),
        pytest.param(['{"x": Inf', f'inity, "data": [{FIRST_RECORD_JSON}]}}'], id="Infinity split"),
        pytest.param(['{"x": Na', f'N, "data": [{FIRST_RECORD_JSON}]}}'], id="NaN split"),
        pytest.param(
            ['{"x": -Infi', f'nity, "data": [{FIRST_RECORD_JSON}]}}'], id="-Infinity split"
        ),
    ],
)
def test_nonfinite_constants_survive_splits(parse_envelope, chunks, first_event):
    result = parse_envelope(chunks)

    assert result == [first_event]


def test_skips_a_leading_byte_order_mark(parse_envelope, first_event):
    # The paged read path parses the raw bytes with json.loads, which strips a UTF-8
    # BOM before parsing; iter_text decodes it through, so the streamed reader skips it.
    body = f'\ufeff{{"data": [{FIRST_RECORD_JSON}]}}'
