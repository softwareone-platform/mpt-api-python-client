...
def test_property_services(products_service, service_method, expected_service_class):
    result = getattr(products_service, service_method)("PRD-001")

    assert isinstance(result, expected_service_class)
    assert result.endpoint_params == {"product_id": "PRD-001"}
...
def test_async_property_services(async_products_service, service_method, expected_service_class):
    result = getattr(async_products_service, service_method)("PRD-001")
...
def test_update_settings(products_service):
    ...
    with respx.mock:
        ...
        result = products_service.update_settings(product_id, settings_data)
...
async def test_async_update_settings(async_products_service):
    ...
    with respx.mock:
        ...
        result = await async_products_service.update_settings(product_id, settings_data)
